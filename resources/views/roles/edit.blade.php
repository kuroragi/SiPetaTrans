...
@section('title', 'Edit Role - SIPETA-TRANS')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-8 py-6">
            ...
            <p class="text-blue-100 mt-2">Perbarui informasi role: <strong>{{ $role->name }}</strong></p>
        </div>
        ...
        <form action="{{ route('roles.update', $role) }}" method="POST" class="p-8">
            ...
            <div class="mb-8 p-6 bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg border-2 border-purple-200">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="flex items-center gap-3 p-3 bg-white rounded">
                        ...
                        <div class="text-sm">
                            <p class="text-gray-600">Nama Role</p>
                    ...
                    <div class="flex items-center gap-3 p-3 bg-white rounded">
                        ...
                        <div class="text-sm">
                            <p class="text-gray-600">Jumlah Permission</p>
                            <p class="font-bold text-gray-800">{{ $role->permissions->count() }} permission</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-8">
                    ...
                    <div class="border-2 border-gray-200 rounded-lg overflow-hidden">
                        ...
                        <div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-2 max-h-72 overflow-y-auto">
                            @foreach($permissions as $permission)
                                @php
                                    $defaultChecked = in_array($permission->id, old('permissions', $rolePermissionIds));
                                @endphp
                                <label class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 cursor-pointer border border-transparent hover:border-blue-200 transition">
                                    ...
                                    <span class="text-sm font-medium text-gray-800 font-mono">{{ $permission->name }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
                @else
                    <div class="p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded">
                        <p class="text-sm text-yellow-800">Belum ada permission yang tersedia.</p>
                    </div>
                @endif
            </div>
            ...
            <div class="mb-8 p-6 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg border-2 border-gray-200">
                ...
                <div class="flex items-center gap-4 p-4 bg-white rounded-lg">
                    ...
                    <div>
                        <p id="previewName" class="font-bold text-gray-800 text-lg">{{ old('name', $role->name) }}</p>
            ...
            <div class="flex gap-3 pt-6 border-t-2 border-gray-200">
                ...
                <a href="{{ route('roles.index') }}" class="flex-1 px-6 py-3 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition font-bold flex items-center justify-center gap-2">
...
<script>
    document.getElementById('name').addEventListener('input', function () {
        document.getElementById('previewName').textContent = this.value || '{{ addslashes($role->name) }}';
    });
