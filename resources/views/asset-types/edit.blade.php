...
<div class="max-w-2xl mx-auto">
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('asset-types.index') }}" class="inline-flex items-center gap-2 text-blue-600 hover:text-blue-800 font-semibold">
        ...
        <form action="{{ route('asset-types.update', $assetType) }}" method="POST" class="p-8">
            ...
            <div class="mb-8 p-6 bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg border-2 border-purple-200">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="flex items-center gap-3 p-3 bg-white rounded">
                        <div class="w-12 h-12 rounded border-4" style="border-color: {{ $assetType->color }}; background-color: {{ $assetType->color }}20;"></div>
                        <div class="text-sm">
                ...
                <div>
                    ...
                    <select id="icon" name="icon" 
                        ...
                        <optgroup label="Informasi & Status">
                            <option value="fa-circle-info" {{ (old('icon') ?? $assetType->icon) === 'fa-circle-info' ? 'selected' : '' }}>fa-circle-info - Informasi</option>
                            <option value="fa-exclamation" {{ (old('icon') ?? $assetType->icon) === 'fa-exclamation' ? 'selected' : '' }}>fa-exclamation - Perhatian</option>
                            <option value="fa-warning" {{ (old('icon') ?? $assetType->icon) === 'fa-warning' ? 'selected' : '' }}>fa-warning - Peringatan</option>
                            <option value="fa-shield" {{ (old('icon') ?? $assetType->icon) === 'fa-shield' ? 'selected' : '' }}>fa-shield - Keamanan</option>
                            <option value="fa-flag" {{ (old('icon') ?? $assetType->icon) === 'fa-flag' ? 'selected' : '' }}>fa-flag - Bendera</option>
                        </optgroup>
                        <optgroup label="Transportasi">
                            <option value="fa-bus" {{ (old('icon') ?? $assetType->icon) === 'fa-bus' ? 'selected' : '' }}>fa-bus - Bus</option>
                            <option value="fa-car" {{ (old('icon') ?? $assetType->icon) === 'fa-car' ? 'selected' : '' }}>fa-car - Mobil</option>
                            <option value="fa-truck" {{ (old('icon') ?? $assetType->icon) === 'fa-truck' ? 'selected' : '' }}>fa-truck - Truk</option>
                            <option value="fa-road" {{ (old('icon') ?? $assetType->icon) === 'fa-road' ? 'selected' : '' }}>fa-road - Jalan</option>
                            <option value="fa-routes" {{ (old('icon') ?? $assetType->icon) === 'fa-routes' ? 'selected' : '' }}>fa-routes - Rute</option>
                            <option value="fa-traffic-light" {{ (old('icon') ?? $assetType->icon) === 'fa-traffic-light' ? 'selected' : '' }}>fa-traffic-light - Lampu Lalu Lintas</option>
                        </optgroup>
                        <optgroup label="Lokasi & Navigasi">
                            <option value="fa-map-marker-alt" {{ (old('icon') ?? $assetType->icon) === 'fa-map-marker-alt' ? 'selected' : '' }}>fa-map-marker-alt - Marker Peta</option>
                            <option value="fa-map" {{ (old('icon') ?? $assetType->icon) === 'fa-map' ? 'selected' : '' }}>fa-map - Peta</option>
                            <option value="fa-map-location-dot" {{ (old('icon') ?? $assetType->icon) === 'fa-map-location-dot' ? 'selected' : '' }}>fa-map-location-dot - Lokasi</option>
                            <option value="fa-location-dot" {{ (old('icon') ?? $assetType->icon) === 'fa-location-dot' ? 'selected' : '' }}>fa-location-dot - Titik Lokasi</option>
                            <option value="fa-compass" {{ (old('icon') ?? $assetType->icon) === 'fa-compass' ? 'selected' : '' }}>fa-compass - Kompas</option>
                        </optgroup>
                        <optgroup label="Orang & Kelompok">
                            <option value="fa-walking" {{ (old('icon') ?? $assetType->icon) === 'fa-walking' ? 'selected' : '' }}>fa-walking - Berjalan</option>
                            <option value="fa-person" {{ (old('icon') ?? $assetType->icon) === 'fa-person' ? 'selected' : '' }}>fa-person - Orang</option>
                            <option value="fa-users" {{ (old('icon') ?? $assetType->icon) === 'fa-users' ? 'selected' : '' }}>fa-users - Kelompok</option>
                            <option value="fa-user-tie" {{ (old('icon') ?? $assetType->icon) === 'fa-user-tie' ? 'selected' : '' }}>fa-user-tie - Profesional</option>
                        </optgroup>
                        <optgroup label="Media & Dokumentasi">
                            <option value="fa-camera" {{ (old('icon') ?? $assetType->icon) === 'fa-camera' ? 'selected' : '' }}>fa-camera - Kamera</option>
                            <option value="fa-video" {{ (old('icon') ?? $assetType->icon) === 'fa-video' ? 'selected' : '' }}>fa-video - Video</option>
                            <option value="fa-eye" {{ (old('icon') ?? $assetType->icon) === 'fa-eye' ? 'selected' : '' }}>fa-eye - Mata/Pengawasan</option>
                            <option value="fa-binoculars" {{ (old('icon') ?? $assetType->icon) === 'fa-binoculars' ? 'selected' : '' }}>fa-binoculars - Binokular</option>
                            <option value="fa-radar" {{ (old('icon') ?? $assetType->icon) === 'fa-radar' ? 'selected' : '' }}>fa-radar - Radar</option>
                        </optgroup>
                        <optgroup label="Perbaikan & Perawatan">
                            <option value="fa-hammer" {{ (old('icon') ?? $assetType->icon) === 'fa-hammer' ? 'selected' : '' }}>fa-hammer - Palu</option>
                            <option value="fa-wrench" {{ (old('icon') ?? $assetType->icon) === 'fa-wrench' ? 'selected' : '' }}>fa-wrench - Kunci Inggris</option>
                            <option value="fa-tools" {{ (old('icon') ?? $assetType->icon) === 'fa-tools' ? 'selected' : '' }}>fa-tools - Alat</option>
                            <option value="fa-toolbox" {{ (old('icon') ?? $assetType->icon) === 'fa-toolbox' ? 'selected' : '' }}>fa-toolbox - Kotak Alat</option>
                            <option value="fa-gears" {{ (old('icon') ?? $assetType->icon) === 'fa-gears' ? 'selected' : '' }}>fa-gears - Gigi</option>
                        </optgroup>
                        <optgroup label="Data & Informasi">
                            <option value="fa-chart-bar" {{ (old('icon') ?? $assetType->icon) === 'fa-chart-bar' ? 'selected' : '' }}>fa-chart-bar - Grafik Batang</option>
                            <option value="fa-chart-line" {{ (old('icon') ?? $assetType->icon) === 'fa-chart-line' ? 'selected' : '' }}>fa-chart-line - Grafik Garis</option>
                            <option value="fa-chart-pie" {{ (old('icon') ?? $assetType->icon) === 'fa-chart-pie' ? 'selected' : '' }}>fa-chart-pie - Grafik Pie</option>
                            <option value="fa-chart-area" {{ (old('icon') ?? $assetType->icon) === 'fa-chart-area' ? 'selected' : '' }}>fa-chart-area - Grafik Area</option>
                        </optgroup>
                        <optgroup label="Lainnya">
                            <option value="fa-cube" {{ (old('icon') ?? $assetType->icon) === 'fa-cube' ? 'selected' : '' }}>fa-cube - Kubus</option>
                            <option value="fa-box" {{ (old('icon') ?? $assetType->icon) === 'fa-box' ? 'selected' : '' }}>fa-box - Kotak</option>
                            <option value="fa-package" {{ (old('icon') ?? $assetType->icon) === 'fa-package' ? 'selected' : '' }}>fa-package - Paket</option>
                            <option value="fa-star" {{ (old('icon') ?? $assetType->icon) === 'fa-star' ? 'selected' : '' }}>fa-star - Bintang</option>
                            <option value="fa-heart" {{ (old('icon') ?? $assetType->icon) === 'fa-heart' ? 'selected' : '' }}>fa-heart - Hati</option>
                            <option value="fa-lightbulb" {{ (old('icon') ?? $assetType->icon) === 'fa-lightbulb' ? 'selected' : '' }}>fa-lightbulb - Ide</option>
                            <option value="fa-sun" {{ (old('icon') ?? $assetType->icon) === 'fa-sun' ? 'selected' : '' }}>fa-sun - Matahari</option>
                            <option value="fa-moon" {{ (old('icon') ?? $assetType->icon) === 'fa-moon' ? 'selected' : '' }}>fa-moon - Bulan</option>
                        </optgroup>
                    </select>
                ...
                <div>
                    ...
                    <div id="colorPreview" class="mb-4 p-4 bg-gray-50 rounded-lg border-2 border-gray-200">
                        ...
                        <div class="flex items-center gap-4">
                            <div id="colorCircle" class="w-16 h-16 rounded-lg border-4 border-gray-300 transition" style="background-color: {{ old('color', $assetType->color) }};"></div>
                            <div>
                                <p id="colorHexDisplay" class="font-mono text-lg font-bold text-gray-800">{{ old('color', $assetType->color) }}</p>
                    ...
                    <div class="flex gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <input type="text" id="colorHex" name="color"
                                ...
                                value="{{ old('color', $assetType->color) }}" placeholder="#000000" onchange="updateColorFromHex()">
                        </div>
                    </div>
            ...
            <div class="mb-8 p-6 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg border-2 border-gray-200">
                ...
                <div class="flex items-center gap-6">
                    <div id="previewCard" class="flex items-center gap-4 p-4 bg-white rounded-lg flex-1">
                        <div id="previewIconContainer" class="w-16 h-16 rounded-lg flex items-center justify-center text-3xl" 
                            style="background-color: {{ old('color', $assetType->color) }}20; color: {{ old('color', $assetType->color) }};">
                            <i class="fas {{ old('icon', $assetType->icon) }}"></i>
                        </div>
                        <div class="flex-1">
                            <p id="previewName" class="font-bold text-gray-800">{{ old('name', $assetType->name) }}</p>
                            <p id="previewDesc" class="text-xs text-gray-600">{{ old('description', $assetType->description) }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-3 pt-6 border-t-2 border-gray-200">
                ...
                <a href="{{ route('asset-types.index') }}" class="flex-1 px-6 py-3 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition font-bold flex items-center justify-center gap-2">
