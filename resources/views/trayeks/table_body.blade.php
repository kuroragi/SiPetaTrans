@forelse($trayeks as $index => $trayek)
    <tr class="hover:bg-gray-50 transition">
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            <div class="text-sm text-gray-900">{{ $trayek->armada_active_count ?? 0 }}</div>
        </td>
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
            <div class="flex gap-2">
                ...
                @endcan
                @can('delete trayeks')
                    <form action="{{ route('trayeks.destroy', $trayek->id) }}" method="POST" class="inline"
                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus trayek ini?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
