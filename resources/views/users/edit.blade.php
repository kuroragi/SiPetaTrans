            ...
            <form action="{{ route('users.update', $user) }}" method="POST" class="p-8">
                        ...
                        <div class="text-sm">
                            ...
                            <div class="flex flex-wrap gap-1 mt-1">
                                @forelse($user->roles as $role)
                                    <span
                                        class="px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">{{ $role->name }}</span>
                                @empty
                ...
                <div class="mb-8 p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="password_confirmation" class="block text-xs font-bold text-gray-700 mb-2">Konfirmasi
                                Password Baru</label>
                        ...
                        <div class="border-2 border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-2">
                                @foreach ($roles as $role)
                                    @php
                                        $defaultChecked = in_array($role->name, old('roles', $userRoleIds));
                                    @endphp
                                    <label
                                        ...
                                        <div>
                                            ...
                                            <span class="text-xs text-gray-400 ml-1">({{ $role->permissions_count }}
                                                permission)</span>
                                        </div>
                                    </label>
                                @endforeach
...
@push('scripts')
    <script>
        ...
        function updateRoles() {
            ...
            document.getElementById('previewRoles').textContent = labels.length ? labels.join(', ') :
