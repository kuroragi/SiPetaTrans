    ...
    <div class="bg-white rounded-lg shadow">
        ...
        <form action="{{ route('assets.update', $asset) }}" method="POST" enctype="multipart/form-data" class="p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select id="status" name="status" 
                            ...
                            <option value="perlu_perbaikan" {{ $asset->status == 'perlu_perbaikan' ? 'selected' : '' }}>⚠️ Perlu Perbaikan</option>
                            <option value="rusak" {{ $asset->status == 'rusak' ? 'selected' : '' }}>❌ Rusak</option>
                            <option value="dalam_pemeliharaan" {{ $asset->status == 'dalam_pemeliharaan' ? 'selected' : '' }}>🔧 Dalam Pemeliharaan</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-sm mt-1"><i class="fas fa-exclamation-circle"></i> {{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" id="last_maintenance" name="last_maintenance" value="{{ $asset->last_maintenance }}"
            ...
            <div class="mb-8 pb-8 border-b">
                ...
                <div class="mt-6 bg-blue-50 rounded-lg p-4 border-l-4 border-blue-500">
                    ...
                    <p class="text-xs text-blue-700 mb-3">
                        <i class="fas fa-info-circle"></i> Klik pada peta untuk mengubah lokasi aset. Latitude dan Longitude akan terupdate otomatis.
                    </p>
                    <div id="mapPreview" style="height: 300px; border-radius: 8px; background-color: #e5e7eb; cursor: pointer;" title="Klik untuk mengubah lokasi"></div>
                </div>
            </div>
            ...
            <div class="flex gap-4 justify-end">
                ...
                <a href="{{ route('assets.show', $asset) }}" class="px-6 py-3 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition font-semibold flex items-center gap-2">
...
<script>
    ...
    function initializeMap() {
        ...
        marker = L.marker([lat, lng]).addTo(map).bindPopup(`<div class="text-sm"><strong>Lokasi Saat Ini:</strong><br>Lat: ${lat.toFixed(6)}<br>Lng: ${lng.toFixed(6)}</div>`);
        ...
        map.on('click', function(e) {
            ...
            // Show popup
            marker.setPopupContent(`<div class="text-sm"><strong>✓ Lokasi Diubah:</strong><br>Lat: ${newLat.toFixed(6)}<br>Lng: ${newLng.toFixed(6)}</div>`).openPopup();
        });
