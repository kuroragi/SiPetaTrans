...
<div class="max-w-6xl mx-auto">
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('assets.index') }}" class="inline-flex items-center gap-2 text-blue-600 hover:text-blue-800 font-semibold">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg shadow-lg p-8 mb-6">
        ...
        <div class="grid grid-cols-4 gap-4">
            <div class="bg-white bg-opacity-10 rounded-lg p-4">
                ...
                <p class="text-2xl font-bold">#{{ $asset->id }}</p>
            </div>
            <div class="bg-white bg-opacity-10 rounded-lg p-4">
                <p class="text-blue-100 text-sm">Jumlah Unit</p>
                <p class="text-2xl font-bold">{{ $asset->quantity ?? 1 }}</p>
            </div>
            <div class="bg-white bg-opacity-10 rounded-lg p-4">
                <p class="text-blue-100 text-sm">Dibuat</p>
                <p class="text-sm font-mono">{{ $asset->created_at->format('d M Y') }}</p>
            </div>
            <div class="bg-white bg-opacity-10 rounded-lg p-4">
                <p class="text-blue-100 text-sm">Diperbarui</p>
                <p class="text-sm font-mono">{{ $asset->updated_at->format('d M Y H:i') }}</p>
            </div>
        </div>
    </div>
                    ...
                    @if($asset->last_maintenance)
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-600">({{ $asset->last_maintenance->diffForHumans() }})</span>
                            </div>
                        </div>
                    @endif
                </div>
        ...
        <div class="space-y-6">
                ...
                <div class="p-6">
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <p class="text-2xl font-bold text-gray-800">{{ ucfirst(str_replace('_', ' ', $asset->status)) }}</p>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    <a href="{{ route('assets.edit', $asset) }}" class="w-full px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-semibold flex items-center justify-center gap-2">
                    ...
                    <a href="{{ route('assets.index') }}" class="w-full px-4 py-3 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition font-semibold flex items-center justify-center gap-2">
